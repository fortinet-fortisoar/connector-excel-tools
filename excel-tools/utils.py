...
def upload_file_to_fortisoar(workbook_dict, create_attachment=True):
    try:
        ...
        logger.warning('Could not upload fil to FortiSOAR'.format(err))
        raise ConnectorError('Could not upload fil to FortiSOAR'.format(err))
...
def validate_json_schema(_instance, _schema):
    try:
        ...
        raise ConnectorError(
            "Entered data: [{0}] does not match the schema:{1}. Error: {2}".format(_instance, _schema, err))
